...
\item[] \ans{
    ...
    Rather than a differential volume with respect to radius and solid angle $\rho^2\sin(\varphi)d\rho d\theta d\varphi$, we consider this shell, already integrated around solid angle, only differential to radius, $4\pi\rho^2d\rho$.\\
    ...
    \begin{equation*}
        \ddt(n)4\pi\rho^2d\rho = \left((\nu_p\Sigma_f-\Sigma_a)\phi + s + \sum_{i=1}^{N}\lambda_iC_i\right)4\pi\rho^2d\rho + \mathrm{current~in-current~out}
    \end{equation*}
    Given that we consider the whole shell, the only two surfaces we need to consider 
