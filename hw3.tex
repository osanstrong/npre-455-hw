...
\item[] \ans{Since all neutrons are moving in the same direction $1\hat{x}$, the angular current is $\vec{j}(\hat{\Omega}{\color{gray},\vec{r},E,t})=nv\delta(\hat{\Omega}-\hat{x})\hat{\Omega}$, where $\delta(\hat{\Omega}-\hat{x})$ is a Dirac delta function centered on the angle equivalent to the vector $\hat{x}$.}
...
\item[] \ans{The net rate crossing the surface at any point with normal $\hat{n}$ (let us assume outward facing means upwards and to the left, so that current looks like it will enter the surface) is $-\vec{J}\cdot\hat{n}$. If we assume the surface is flatly angled at $\varphi$, and since our current is constant, that will evaluate for all points on the surface to:
\begin{align*}
    -\vec{J}\cdot\hat{n} &= -\int_{4\pi}d\hat{\Omega}\vec{j}(\hat{\Omega}{\color{gray},\vec{r},E,t}) \\
    &= -\int_{4\pi}d\hat{\Omega}nv\delta(\hat{\Omega}-\hat{x})\hat{\Omega}\\
    &=-nv\hat{x}\cdot\hat{n} \\
    &= -nv \times \hat{n}_x \\
    &= nv\sin(\varphi)
\end{align*}
...
\item[] \ans{
    Assuming diffusion theory applies:
    \begin{equation*}
        \vec{J}=D\grad{\phi}
    \end{equation*}
    Applying the gradient in spherical coordinates (but 1D, only considering r):
    \begin{align*}
        \vec{J} &= -D\left(\frac{\partial\phi}{\partial r}\hat{r}\right) \\
        \color{gray}\rightarrow&\color{gray}\left[\frac{f(x)}{g(x)}\right]'=\frac{f'g-fg'}{g^2},f(r)=\phi_0R\sin{\left(\frac{\pi r}{R}\right)}, g(r)=r \\
        &\color{gray}f'(r) = \phi_0\pi\cos{\left(\frac{\pi r}{R}\right)},g'(r)=1\\
        \vec{J}&= -D\left(\frac{\phi_0\pi\cos{\left(\frac{\pi r}{R}\right)}r - \phi_0R\sin{\left(\frac{\pi r}{R}\right)}}{r^2}\right)\hat{r} \\
        \vec{J}&=\boxed{ -\frac{D\phi_0}{r^2}\left(\pi\cos{\left(\frac{\pi r}{R}\right)}r - R\sin{\left(\frac{\pi r}{R}\right)}\right)\hat{r}} \\
    \end{align*}
}
...
\item[] \ans{
    See Figure~\ref{fig:curr-plot}. At $r=0$, while the value is technically not directly defined, it approaches 0. This makes sense, because if the problem is rotationally symmetric, there couldn't be any net neutron movement at exactly $r=0$ for any angle $\hat{\Omega}$, since that would make $\vec{J}\cdot\hat{r}$ the reverse value on the opposite angle $-\hat{\Omega}$
}
\item[] \ans{\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{figures/hw3_JPlot.png}
    \caption{\ans{The radial current $\vec{J}\cdot\hat{r}$ from $r=0$ to $r=50\mathrm{cm}$. Note that while behavior of the function is technically undefined at $r=0$, it approaches 0.}}
    \label{fig:curr-plot}
\end{figure}}
\item[(c)] ({\bf 5 points}) What is the rate (1/s) of neutrons which leak from the surface of the sphere?
\item[] \ans{
\item[] The rate of neutrons leaving the sphere may be found by integrating the dot product of the neutron current and the surface normal over the surface. \\

    If we assume $\vec{J}(r)$ is still constant w.r.t angular coordinates, then the integral of this current crossing the surface evaluates:
    \begin{gather*}
        \int_{S}\vec{J}\cdot\hat{r}dA_{r=R} \\
        4\pi R^2 J_r \\
        -4R^2\pi \left[\frac{D\phi_0}{R^2}\left(\pi\cos{\left(\frac{\pi R}{R}\right)}R - R\sin{\left(\frac{\pi R}{R}\right)}\right)\right]\\
        -4R\pi \left[D\phi_0\left(\pi\cos{\left(\pi\right)} - \sin{\left(\pi\right)}\right)\right]\\
        -4R\pi \left[D\phi_0\left(\pi(-1)\right)\right]\\
        \boxed{4R\pi^2 D\phi_0}\\
        4\times50\mathrm{cm}\times\pi^2\times1.1\mathrm{cm}\times13.1\cdot10^{13}\mathrm{cm^{-2}s^{-1}} \\
        \boxed{2.844\times10^{17}/\mathrm{s}}
    \end{gather*}

    Where this corresponds to neutrons crossing in the outwards facing, positive-$r$ direction.
}
\item[(d)] ({\bf 5 points}) What is the net number of neutrons crossing the spherical surface at $r=R/2$ per second?
\item[] \ans{
    We can imagine the arbitrary surface at $r=R/2$, and the net rate at which neutrons cross that surface can still be found by integrating the product of the current there and the normal vector:
    \begin{gather*}
        \int_{S}\vec{J}\cdot\hat{r}dA_{r=R/2} \\
        4\pi \frac{R^2}{4} J_r \\
        \pi R^2 \left[-\frac{4D\phi_0}{R^2}\left(\pi\cos{\left(\frac{\pi R}{2R}\right)}R/2 - R\sin{\left(\frac{\pi R}{2R}\right)}\right)\right]\\
        \pi R \left[-4D\phi_0\left(\pi\cos{\left(\frac{\pi}{2}\right)}/2 - \sin{\left(\frac{\pi}{2}\right)}\right)\right]\\
        \pi R \left[-4D\phi_0\left(0-1\right)\right]\\ 
        \boxed{4R\pi D\phi_0}\\ 
        4\times50\mathrm{cm}\times\pi\times1.1\mathrm{cm}\times13.1\cdot10^{13}\mathrm{cm^{-2}s^{-1}} \\
        \boxed{9.054\cdot10^{16}/\mathrm{s}}
    \end{gather*}

    Where this corresponds to neutrons crossing in the outwards facing, positive-$r$ direction. It makes sense that this is less than the neutrons crossing at the outside, even though the current is greater: it's greater for points on this surface which is a fraction of the surface area of the outermost surface.
}
...
\item[] \ans{
    Let us assume that the sphere is thin enough that flux is constant across any point $\vec{r}\in R/2-0.5\text{cm}<r<R/2+0.5\text{cm}$. \\
    We can then model neutron absorption and generation, volume-integrated functions of flux, as simply the shell volume times a uniform rate:
    \begin{align*}
        (i) &= \Sigma_a \phi(r=R/2) V_{shell}\\
        &\color{gray}\phi(r=R/2) = \frac{2\phi_0R\sin{\left(\frac{\pi}{2}\right)}}{R}=2\phi_0=26.2\times10^{13}\mathrm{cm^{-2}s^{-1}}\\
        &\color{gray}V_{shell}=\frac{4\pi}{3}(r_o^3-r_i^3)=\frac{4\pi}{3}(25.5^3-24.5^3)\mathrm{cm^3}=7855.03\mathrm{cm^3}\\
        &\color{gray}\Sigma_a=0.108\mathrm{cm^{-1}}\\
        (i) &= 0.108\mathrm{cm^{-1}}\times 26.2\times10^{13}\mathrm{cm^{-2}s^{-1}} \times 7855.03\mathrm{cm^3}=\boxed{2.22\times10^{17}/\mathrm{s}}\\
        (ii) &= \nu\Sigma_f\phi(r=R/2)V_{shell}\\&=2.4\times0.0727/\mathrm{cm}\times26.2\times10^{13}\mathrm{cm^{-2}s^{-1}} \times 7855.03\mathrm{cm^3}=\boxed{3.59\times10^{17}/\mathrm{s}}\\
    \end{align*}

    To find the number of neutrons entering and leaving, the \texttt{python} script \texttt{plot\_p8.py} is shown in Appendix~\ref{sec:p8e-script}, and used to evaluate the previously found crossing formula for the two specific $r$ values more concisely:
    
    \begin{align*}
        C(r)&=4\pi r^2\times \vec{J}\cdot\hat{r} \\
            &=\mathtt{4**r*get\_Jr(r)}\\
        (iii)C_{in}&=C(R/2-0.5)=\mathtt{8.611809974870085e+16}=\boxed{0.8612\times10^{17}/s} \\
        C_{out}&=C(R/2+0.5)=\mathtt{9.505263884593955e+16}=\boxed{0.9505\times10^{17}/s} \\
    \end{align*}
    This yields a neutron balance 
    \begin{gather*}
        \mathrm{gen.-abs.+in-out} \\
        \left[3.59 - 2.22 + 0.86 - 0.95\right]\times10^{17}/\mathrm{s}\\
        1.28\times10^{17}/\mathrm{s}\\
    \end{gather*}
    Which is not zero. 
    Given that neutron production and absorption are proportional to the same flux at a point under these uniform properties, undoing the thin volume assumption and integrating over the actual non-uniform function of flux for each point might help, but I'm not sure that it would have this big of an effect.
    Another possible cause, which I would have been able to address with more time, would have been if my formulation of current was somehow missing a factor. (I have spent a couple hours redoing things and testing it in Desmos (https://www.desmos.com/calculator/kdmohf9x1f), and been unable to find a key issue.)
    If, for example, the diffusion constant were $D=17.098cm$, then the net current would be enough to account for the extra neutron production, but that's a matter 
}
\end{enumerate}
...
\begin{gathered}
    \int_{4\pi}d\hat{\Omega}f(\theta,\varphi) \\
    \int_{0}^{2\pi}\int_{0}^{\pi}f(\theta,\varphi)\sin{(\theta)}d\theta d\varphi \\
    \int_{0}^{2\pi}\int_{0}^{\pi}\theta\varphi^2\sin{(\theta)}d\theta d\varphi \\
    \int_{0}^{2\pi}\varphi^2\int_{0}^{\pi}\theta\sin{(\theta)}d\theta d\varphi \\
    \color{gray}\mathrm{Integrate\ by\ parts,\ where\ }uv=-\theta\cos(\theta) \\
    \int_{0}^{2\pi}\varphi^2
    \left(\left.\left[-\theta\cos(\theta)\right]\right|_0^\pi - 
    \int_{0}^{\pi}-\cos{(\theta)}d\theta\right)
    d\varphi\\
    \int_{0}^{2\pi}\varphi^2
    \left(\pi + 0 + \left.[\sin(\theta)]\right|_0^\pi\right)
    d\varphi\\
    \int_{0}^{2\pi}\varphi^2\pi d\varphi\\
    \pi\int_{0}^{2\pi}\varphi^2 d\varphi\\
    \pi\left.\left[\frac{1}{3}\varphi^3\right]\right|_0^{2\pi}\\
    \pi\frac{8\pi^3}{3}\\
    \boxed{\frac{8\pi^4}{3}}
\end{gathered}
\end{equation*}
...
\appendix
\section{Python Utility Script for Problem 8}\label{sec:p8e-script}
\ans{
The contents of the \texttt{python} script \texttt{plot\_p8.py} may be found:}
\begin{verbatim}
