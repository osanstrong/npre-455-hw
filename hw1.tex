...
\item[(a)] ({\bf 10 points}) Solve Eq. \eqref{eq:one1} subject to the boundary condition $N(0)=N_0$.
    ...
    \begin{align*}
        ...
        N(t)e^{\lambda t} = \int f(t)e^{\lambda t}dt \\
        N(t) = e^{-\lambda t} \left(\int f(t)e^{\lambda t}dt \right)\\
...
\begin{itemize}
    \item Let us start with the homogenous portion of \ref{eq:one3}:
    \begin{equation}
        \label{eq:phi_h}
        \frac{d^2\phi_h(x)}{dx^2}-a^2\phi_h(x)=0
    \end{equation}
    This may have a general hyperbolic Helmholtz solution:
    \begin{equation}
        \phi_h(x) = C_1cosh(ax) + C_2sinh(ax)
    \end{equation}
    We will come back to the homogenous $\phi_h(x)$. In the meanwhile allow us to find a particular solution $\phi_p(x)$.\\
    Since the inhomogeneous portion $2x^3$ is purely polynomial, we shall assume a $\phi_p(x)$ of an arbitrary polynomial:
    \begin{align*}
        \phi_p(x) &= Ax^3 + Bx^2 + Cx + d \\ 
        \phi_p''(x) &= 6Ax + 2B \\
        \phi_p''(x) - a^2\phi_p(x) = 2x^3 &= (-2a^2A)x^3 -a^2Bx^2 + (6A-Ca^2)x + (2B-a^2D) \\
        -a^2Ax^3 &= 2x^3 \\
        &\rightarrow A = -\frac{2}{a^2} \\
        -a^2Bx^2 &= 0 \\
        &\rightarrow B = 0 \\
        6Ax - a^2Cx = -\frac{12}{a^2}x - a^2Cx &= 0 \\
        &\rightarrow C = -\frac{12}{a^4} \\
        2B - a^2D = -a^2D &= 0 \\
        &\rightarrow D = 0\\
    \end{align*}
    \begin{equation}
        \label{eq:phi_p}
        \rightarrow \phi_p(x) = -\frac{2}{a^2}x^3 - \frac{12}{a^4}x
    \end{equation}
    Combining the two equations \ref{eq:phi_h} and \ref{eq:phi_p} yields a general solution $\phi$, which can be evaluated for the given boundary conditions:
    \begin{align*}
        \phi(x) = \phi_h(x) + \phi_p(x) &= C_1cosh(ax) + C_2sinh(ax) -\frac{2}{a^2}x^3 - \frac{12}{a^4}x \\
        \phi(0) = A &= C_1cosh(0) + C_2sinh(0) - 0 - 0 \\
        &\rightarrow C_1 = A\\
        \phi(L) = B &= Acosh(aL) + C_2sinh(aL)-\frac{2}{a^2}L^3 - \frac{12}{a^4}L \\
        C_2sinh(aL) = B - Acosh(aL) + \frac{2}{a^2}L^3 + \frac{12}{a^4}L \\
        &\rightarrow C_2 = \frac{B - Acosh(aL) + \frac{2}{a^2}L^3 + \frac{12}{a^4}L}{sinh(aL)} \\
    \end{align*}
    Yielding a final $\phi(x)$ of
    \begin{equation}
        \phi(x) = Acosh(ax) + \frac{B - Acosh(aL) + \frac{2}{a^2}L^3 + \frac{12}{a^4}L}{sinh(aL)}sinh(ax) -\frac{2}{a^2}x^3 - \frac{12}{a^4}x 
    \end{equation}
\end{itemize}
\end{enumerate}
...
\begin{itemize}
    \item \textbf{False.} In some reactions, kinetic energy may be converted to other forms, like inelastic scattering reactions, which might result in the nucleus being excited and subsequently releasing a smaller particle like gamma radiation.
\end{itemize}
...
\begin{itemize}
    \item This approach is incorrect. 
    As the neutrons are specified to be very slow, their wavelengths may be on the scale of atomic bonds and molecules.
    A correct approach would involve using Thermal Scattering Laws (TSLs), recorded data corrections for low energies where interactions with the bond itself start to make the cross section different than that of just the same atoms individually.
\end{itemize}
\end{enumerate}
...
\begin{enumerate}
\item[(a)] ({\bf 20 points}) Predict the tritium production rate $\dot{T}$, in units of mg/year, from the above reaction in a light water reactor. Assume that macroscopic cross sections are not a function of time. Parameters describing the reactor are provided below. Report your answer to THREE places after the decimal point.

\begin{itemize}
\item Average thermal neutron flux: $\phi=10^{13}$ 1/cm$^2$/s
\item Average water density: 750 kg/m$^3$ ($\rho$)
\item Volume occupied by water: 9.54 \si{\cubic\meter} ($V$)
\item $\ce{^2_1 H}$ atomic concentration in elemental (natural) hydrogen: 0.015\%
\item Thermal neutron capture cross section for $\ce{^2_1 H}$: 506 $\mu$
\end{itemize}
Additionally, assume
\begin{itemize}
\item Molecular mass of water: 18.015 g/mol ($M_w$)
\item Atomic mass of tritium: 3.016 g/mol ($M_t$)
\end{itemize}
Suppose a uniform reaction rate $R=\phi \Sigma_{c,D}$, where $\Sigma_{c,D}=N_D\sigma_{c,D}$.
\begin{align*}
    N_D &= \frac{D}{H} \times N_H \\
    &= \frac{D}{H} (2N_{H_2O}) \\
    &= 2\frac{D}{H} (\frac{\rho}{M_w}) \\
    &= 2\times 0.00015\times \frac{750 \mathrm{kg/m^3}\times 1000 \mathrm{g/kg}\times 0.00,0001\mathrm{m^3/cm^3}}{18.015\mathrm{g/mol}}\\
    N_D &= 0.00001249 \mathrm{mol/cm^3} \\
    R &= \phi N_D\sigma_{c,D} \\
    &= 10^{13}\frac{1}{\mathrm{cm^2\cdot s}} \times 1.249\cdot10^{-5}\frac{mol}{\mathrm{cm^3}} \times 506 \mu\mathrm{b}\frac{1\mathrm{cm^2}}{10^{30}\mathrm{\mu b}}\\   
    R &= 6.3197\cdot10^{-20} \mathrm{\frac{mol}{cm^3\cdot s}}
\end{align*}
If uniform water is assumed, a production rate can be found by multiplying this reaction rate across the whole volume of water:
\begin{align*}
    \dot{T} &= RVM_t \\
    &=6.3197\cdot10^{-20} \mathrm{\frac{mol}{cm^3\cdot s}} \times 9.54 \mathrm{m^3\cdot\frac{10^6 cm^3}{m^3}} \times 3.016\mathrm{g/mol} \\
    &= 1.818\cdot10^{-12} \mathrm{g/s}\cdot\frac{1000\mathrm{mg}}{\mathrm{g}} \cdot \frac{60\cdot60\cdot24\cdot365.25\mathrm{s}}{\mathrm{year}} \\
    \dot{T} &= 0.057 \mathrm{mg/year}
\end{align*}
\end{enumerate}
